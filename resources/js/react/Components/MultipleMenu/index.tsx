...
export default function MultipleMenu({ texto, slug, posts }: { texto: string; slug: string; posts: Post[] }) {
    ...
    return (
        <div>
            <Link href={`/referencia/${slug}`} className={`${styles.multiple} ${active ? styles.active : ''}`}>
                <div>{texto} <img src={chevron} alt='Leia!' /></div>
            </Link>
            {active && posts.map(post =>
                <Link key={post.id} href={`/referencia/${post.slug}`} className={`${styles.singleChild} ${activeChild?.id === post.id ? styles.active : ''}`}>
